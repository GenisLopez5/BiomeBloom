...
sf::Text& Canvas::addText(string textName, string content, string fontDir, unsigned int size, sf::Vector2f pos, sf::Vector2f anchor)
{
    sf::Font f;
...
sf::Sprite Canvas::addSprite(string spriteName, string textureDir, sf::Vector2f pos, sf::Vector2f anchor, float scale = 1)
{
    ...
    SpriteMap[spriteName].setOrigin(anchor.x*t.getSize().x, anchor.y*t.getSize().y);
    SpriteMap[spriteName].setPosition(pos);
    SpriteMap[spriteName].setScale(sf::Vector2f(scale, scale));
    return SpriteMap[spriteName];
}
void Canvas::removeText(string textName)
{
    TextMap.erase(textName);
}
void Canvas::removeSprite(string spriteName)
{
...
const map<string, sf::Text>& Canvas::get_text_map() const
{
    return TextMap;
}