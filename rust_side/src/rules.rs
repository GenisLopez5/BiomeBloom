...
pub fn find_neighbours(index: usize, buffer: *mut Atom, width: usize, height: usize) -> [EntityTag; 8] {
    let (x, y) = i_to_xy(index, width);
    let mut counter = 0;
    let mut result: [EntityTag; 8]= [0; 8];
    for i in 0..3 {
        for j in 0..3 {
            let x1: usize = (x + width  - 1 + j) % width;
            let y1: usize = (y + height - 1 + i) % height;
            if i != 1 || j != 1 {
                unsafe { result[counter] = (*buffer.add(xy_to_i(x1, y1, width))).entity_tag }
                counter += 1;
            }
        }
    }
...
fn test_neighbours() {
    ...
    dbg!(tags);
    assert_eq!(tags, [9, 10, 11, 1, 3, 5, 6, 7]);
}